...
namespace chess{
    ...
    public:
        ...
        virtual bool isMoveLegal(const Move& move, const BoardView& BoardView, GameState GameState) const = 0; 
        virtual std::unique_ptr<MovementLogic> clone() const = 0;
    };

    class KingMovement : public MovementLogic{
    public:
        bool isMoveLegal(const Move& move, const BoardView& BoardView, GameState GameState) const override;
